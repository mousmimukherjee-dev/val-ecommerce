...
const Cart = () => {
  ...
  const subTotalRounded = Math.floor(subTotal)
  const afterDiscount = Math.floor(subTotal * 0.15);

  const totalAmount = Math.floor(subTotal - afterDiscount);

  return (
    <>
      <div className="grid font-body grid-cols-2 md:grid-cols-3 items-center text-gray-dark p-6 max-w-6xl">
      ...
      {cart.length > 0 && (
        ...
        >
          ...
          <div className="border flex flex-col items-start border-gray-dark p-8 w-full md:w-[80%] h-auto">
            <h2 className="text-center mb-6">Order Summary</h2>

            <div className="flex justify-between w-full">
              <p>Subtotal</p>
              <p data-testid="sub-total-price">{subTotalRounded} SEK</p>
            </div>
